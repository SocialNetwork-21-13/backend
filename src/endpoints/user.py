...
from fastapi import APIRouter, Body, Depends, File, Response, Security, status
...
def upload_file(users: UserRepository = Depends(),
                credentials: HTTPAuthorizationCredentials = Security(security), file: bytes = File()):
...
def upload_default_img(users: UserRepository = Depends(), file: bytes = File()):
    users.upload_image(file)


@router.put("/name_surname", response_description="Update name and surname", response_model=User)
def update_name_and_surname(name: str, surname: str, users: UserRepository = Depends(),
...
def update_user_username(username: str, users: UserRepository = Depends(),
                         credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def delete_user(users: UserRepository = Depends(), credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
    users.delete(user["_id"])


@router.get("/get_image", response_description="Get profile image")
def get_profile_image(users: UserRepository = Depends(),
                      credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def update_bio(bio: str, users: UserRepository = Depends(),
               credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def subscribe_user(sub_id: str, users: UserRepository = Depends(),
                   credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def unsubscribe_user(sub_id: str, users: UserRepository = Depends(),
                     credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def get_subscribers(users: UserRepository = Depends(), credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def get_subscriptions(users: UserRepository = Depends(),
                      credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def like(post_id: str, users: UserRepository = Depends(),
         credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
...
def dislike(post_id: str, users: UserRepository = Depends(),
            credentials: HTTPAuthorizationCredentials = Security(security)):
    user = users.get_current_user(credentials.credentials)
